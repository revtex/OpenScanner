...
Returns bookmarked calls with full metadata. Requires `Authorization: Bearer <token>`. Limited to 100 most recent bookmarked calls.
...
**Error responses:** `401` (no JWT), `500` (server error).
...
All activity endpoints require `Authorization: Bearer <jwt>` with `admin` role. Returns `401` if JWT is missing/invalid, `403` if role is not `admin`, `500` on server error.

#### `GET /api/admin/activity/stats`

Returns aggregated call counts and server status.
...
Returns call counts in hourly buckets for the last 24 hours.
...
{
  "buckets": [
    { "hour": 1700000000, "count": 12 },
    { "hour": 1700003600, "count": 8 }
  ]
}
```

The `hour` values are Unix timestamps at the start of each hour.

#### `GET /api/admin/activity/top-talkgroups`

Returns top 10 talkgroups by call count in the last 24 hours.
