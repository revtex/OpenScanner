...
**Applies to:** Dead code, unused imports, stale files. Never changes functionality, behaviour, or public APIs.
...
- For every candidate removal, run Grep or Glob across the whole workspace to confirm zero references. Include `_test.go` files, generated code, and string-based references (reflection, route registration, config keys).
- Use the Grep tool. If the terminal is genuinely needed, use `rg` — never plain `grep`.
...
8. **Always verify before removing.** Use Grep to confirm a function, variable, or import has zero references before deleting it. If in doubt, leave it.
