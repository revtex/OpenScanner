...
func TestCallHandler_Limiter_CleansUpStaleEntries(t *testing.T) {
	h := NewCallHandler(&db.Queries{}, (*audio.Processor)(nil), (*ws.Hub)(nil), nil, nil)
...
func TestCallHandler_ShareLimiter_CleansUpStaleEntries(t *testing.T) {
	h := NewCallHandler(&db.Queries{}, (*audio.Processor)(nil), (*ws.Hub)(nil), nil, nil)

	staleStart := time.Now().Add(-3 * rateWindowDuration)
	for i := int64(1); i <= 101; i++ {
		h.shareLimiters[i] = &apiKeyLimiter{
			windowStart: staleStart,
	...
	if got := len(h.shareLimiters); got != 1 {
		t.Fatalf("post-sweep shareLimiters size = %d, want 1", got)
	}
}
