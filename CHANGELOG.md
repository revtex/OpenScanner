...
- **Background audio control reworked: mobile only, labelled, and honest about its state.** It was an unlabelled phone icon sitting in the toolbar's icon row, where the row below it labels every control — so it read as a stray button with no name. It is now a labelled **BKGND** control in the mode row alongside LIVE / HOLD / AVOID / SELECT / SEARCH, and it only appears on iOS and Android: a desktop browser keeps a background tab running and plays each call normally, so the stream buys nothing there and the control was pure clutter. It also distinguishes *enabled* from *playing*. Autoplay policy refuses a stream opened without a user gesture, so after a page refresh the preference could be on while nothing was streaming — and because the button looked active, pressing it switched the preference off, leaving off-then-on as the only way to recover. The control now shows a distinct waiting state, and pressing it in that state resumes the stream instead of turning the setting off.
