...
- **AVOID no longer leaks into the saved selection.** Avoiding a talkgroup also switched it off in the stored talkgroup selection, so a *timed* avoid became a permanent disable if the tab was closed before it expired — the talkgroup stayed silent with no badge left to explain why. Avoids are now tracked purely as avoids (they are persisted separately and already mute playback on their own), an expiring avoid no longer force-enables a talkgroup the user had deliberately turned off, and the checkbox renders unchecked while an avoid is active. This also removes a spurious save fired on every login that had an active avoid.
