...
- **Lock-screen now-playing info and artwork while streaming.** In background-audio mode the server owns playback, so nothing on the page was starting a call and the OS media session sat blank — on iOS that meant no talkgroup name and a grey tile. Incoming calls from the WebSocket now label the media session (talkgroup, system, group/tag) after the same selection and AVOID filtering the server applies, so the lock screen and Control Center show what you are hearing. The lock-screen play/pause transport drives the stream in that mode rather than the idle local player, and resuming re-opens the stream rather than continuing from a stale buffered position. OpenScanner also ships app icons for the first time (192px and 512px), used both as the media-session artwork and to fill in the previously empty `icons` array in `manifest.json`, so installing to a home screen gets a real icon too.
- **Background audio and LIVE no longer fight over the audio session.** iOS lets only one media element hold the audio session, and the per-call player re-played its own element on every tap as part of its autoplay unlock — taking the session straight back off the stream and leaving both silent. Enabling background audio now fully releases the local player (its unlock listeners come off, its queue is dropped and its element is released) instead of merely skipping playback, and the LIVE control is disabled while streaming since it would otherwise look active while doing nothing. Background audio always starts from a tap on BKGND, because autoplay policy refuses a stream opened without a user gesture.
- **Continuous listener audio stream, for background playback on mobile.** A new opt-in `GET /api/v1/listener/stream` serves one never-ending MPEG audio response — silence padded with calls as they arrive — instead of one HTTP request per call. iOS only keeps a backgrounded page alive while audio is actively playing, and a scanner is silent between calls, so a locked phone gets suspended in the gap and never plays the next call; a stream that never stops keeps the audio session open, which is how native scanner apps behave. Enable it with the new **Background audio** toggle in the scanner toolbar, which hands playback to the stream and stops the browser playing calls itself. The server applies the listener's saved talkgroup selection, AVOID entries and system grants per call, so the stream carries exactly what that listener would otherwise have heard. Calls are transcoded once and the resulting frames shared across listeners rather than running an encoder per connection, and a listener that falls behind drops its oldest queued audio rather than growing without bound. Requires an FFmpeg build with `libmp3lame`; where that is missing the endpoint answers 503 and everything else is unaffected. Two caveats: HOLD is not applied, because it is transient UI state the server never sees, and while backgrounded the call list and transcripts still catch up only when you return to the page — the stream fixes audio continuity, not the UI's.
...
- **The background-audio control no longer shows up enabled on a fresh page load.** The preference was remembered across reloads, but a stream can only ever be opened from a tap — so a remembered "on" was a setting the page could not act on: it suspended the normal per-call player and drew an enabled-looking control while nothing played, turning from amber to green only when some unrelated click happened to satisfy the gesture. Background audio now always starts off and is switched on deliberately.
- **The background-audio control is no longer hidden by Chrome's "Desktop site" option.** The control is offered only on phones and tablets, which was decided from the user agent alone — and that switch replaces Android's user agent with a desktop one, so the button vanished from a phone that still needed it, with nothing on screen to explain why. Platform detection now also consults the primary pointer, which describes the hardware and is not rewritten. A laptop with both a touch screen and a trackpad still counts as a desktop.
- **The queue counter reads `Q: —` while background audio is on**, instead of a permanent `Q: 0`. The counter tracks the browser's own queue, which is empty by definition in that mode because the server does the queueing; zero read as "nothing waiting" rather than "not applicable here".
- **LIVE and background audio are now one visible choice instead of a button plus an unexplained switch.** They are two ways of listening, so they are drawn as a joined pair — pick LIVE to play in the tab, pick BKGND to keep playing when the screen locks — and selecting one leaves the other. That replaces the greyed-out LIVE button, which looked broken rather than mutually exclusive, and gives the background control a context that explains what it is for. On desktop it is still simply the LIVE button.
- **The volume slider now controls the server stream.** It only ever moved the local player's gain, so in background-audio mode — the one mode where the stream is the thing making sound — it did nothing. (iOS ignores software volume entirely; this fixes Android and desktop.)
- **HOLD is disabled while background audio is on.** HOLD is transient UI state the server never receives, so it cannot filter the stream; it previously looked usable and silently did nothing.
- **Playing a call from search or bookmarks no longer competes with the stream for the audio session.** It started a second media element alongside the stream, which on iOS — where only one element may hold the audio session — is the same conflict that made LIVE and the stream fight. The stream now pauses for the duration of the chosen call and re-opens at live when it ends.
- **The display and lock screen no longer jump to the next call while a long call is still playing.** A call's cue fires when the server starts *sending* it, so a call that arrives while a long one is still going is queued behind it and its cue legitimately does not fire for up to a minute. The 10-second fallback meant to cover a *lost* cue was firing on those legitimately delayed ones, labelling the next call early — the exact confusion the cues were added to remove. The fallback now only applies to calls with no cue at all, and once a page has seen any cue it waits well past the server's maximum queue depth before giving up on one.
- **Pause, skip and replay are disabled while background audio is on.** They act on the local player, which is released when the server stream owns playback, so they looked available while doing nothing. They now grey out with a tooltip, matching how LIVE already behaves in that mode.
- Pressing the background-audio control to resume a waiting stream no longer switches it off instead. The armed gesture listener fires on `mousedown`, which reaches the document before the button's own click, so the stream had already started by the time the press was handled and was read as a press to stop it.
- **Background audio control reworked: mobile only, labelled, and honest about its state.** It was an unlabelled phone icon sitting in the toolbar's icon row, where the row below it labels every control — so it read as a stray button with no name. It is now a labelled **BKGND** control in the mode row alongside LIVE / HOLD / AVOID / SELECT / SEARCH, and it only appears on iOS and Android: a desktop browser keeps a background tab running and plays each call normally, so the stream buys nothing there and the control was pure clutter. It also distinguishes *enabled*, *connecting* and *playing*. Autoplay policy refuses a stream opened without a user gesture, so after a page refresh the preference could be on while nothing was streaming — and because the button looked active, pressing it switched the preference off, leaving off-then-on as the only way to recover. The control now shows a distinct waiting state only when the stream is genuinely waiting on the user, and pressing it there resumes instead of turning the setting off. A stream that is merely connecting (~100 ms) or reconnecting after a drop counts as active, so switching the control on no longer flashes "paused" on its way to "streaming".
- **The scanner display no longer sits empty while the server stream is playing.** In background-audio mode the page plays nothing itself, so nothing set the current call and the display showed no talkgroup, no history and a "Tap LIVE to start listening" hint — for a mode in which LIVE is deliberately disabled. The display is now driven by the same stream cue that drives the lock screen, so the call on screen matches the audio rather than running seconds ahead of it, and the idle hint reads "Background audio — streaming".
- **Lock-screen labels now change in step with the audio you are hearing, not several seconds ahead of it.** In background-audio mode the call event arrives over the WebSocket the moment the call is ingested, but its audio is still inside the stream's buffer — measured at a steady ~8.4 s on the wire — so the talkgroup on the lock screen changed long before the sound did. The server now knows where each call begins on each listener's own stream timeline and sends that position in a new `stream.cue` WebSocket event; the page holds the label until its own playback position reaches it. Because the stream is paced in real time and the player's clock tracks it 1:1, this stays in step on any browser regardless of how much it buffers, and it needs no guessed delay. Cues are per connection (each tab gets its own id), are emitted when a call's first frame is actually sent rather than when it is queued — so a backlog trim cannot leave a stale offset behind — and a call whose cue never arrives is labelled anyway after 10 s, so a dropped WebSocket or an older server degrades to the previous behaviour instead of a blank lock screen.
- Lock-screen now-playing title shows the talkgroup **name** ("Mentor Police") rather than its terse radio alias ("43-ME PD"), falling back to the alias for talkgroups that have no name.
- **Live audio no longer goes silent until you play a call from search.** The gesture that unlocks playback calls `play()` on the shared `<audio>` element while it still has no source. That promise does not reject — it stays *pending* until a source arrives — so it settled at the exact moment the first live call took the element, and the unlock's paired `pause()` then stopped that call dead. Because `ended` never fires on a paused element, the queue never advanced: every later call arrived, appeared in the UI, and piled up in silence. Playing anything from search replaced the stuck item and playback recovered for the rest of the session, which is what made it look like search "fixed" live audio. The unlock now only undoes itself while the element is still source-less and no call has claimed it.
